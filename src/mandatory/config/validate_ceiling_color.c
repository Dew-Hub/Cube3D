...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   validate_ceiling_color.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: buonturk <buonturk@student.42kocaeli.co    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/15 16:17:29 by buonturk          #+#    #+#             */
/*   Updated: 2025/01/15 16:17:31 by buonturk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_comma_syntax(t_c3d *cub3d)
{
	...
	counter = 0;
	c = 0;
	while (cub3d->config->ceiling_color->string[c] != '\0')
	{
		if (cub3d->config->ceiling_color->string[c] == ',')
			counter++;
		c++;
	}
	if (counter != 2)
		report_error(cub3d, "C take a value in RGB format. \
...
static void	check_red_color(t_c3d *cub3d)
{
	...
	if (red_color_length == 0)
		report_error(cub3d, "Red color of C can't be empty.");
	if (red_color_length > 3)
		report_error(cub3d, "Red color of C must be between \
			1-3 characters. (1 - 10 - 100).");
	c = 0;
	while (c < red_color_length)
	{
		if (!ft_is_digit(red_string[c]))
			report_error(cub3d, "Red color of C must consist \
			of digit only. (0 - 9)");
		c++;
	}
	red = ft_atoi(red_string);
	if (red < 0 || red > 255)
		report_error(cub3d, "Red color of C must be a value between 0-255.");
...
static void	check_green_color(t_c3d *cub3d)
{
	...
	if (green_color_length == 0)
		report_error(cub3d, "Green color of C can't be empty.");
	if (green_color_length > 3)
		report_error(cub3d, "Green color of C must be between \
		1-3 characters. (1 - 10 - 100).");
	c = 0;
	while (c < green_color_length)
	{
		if (!ft_is_digit(green_string[c]))
			report_error(cub3d, "Green color of C must consist \
			of digit only. (0 - 9)");
		c++;
	}
	green = ft_atoi(green_string);
	if (green < 0 || green > 255)
		report_error(cub3d, "Green color of C must be a value between 0-255.");
...
static void	check_blue_color(t_c3d *cub3d)
{
	...
	if (blue_color_length == 0)
		report_error(cub3d, "Blue color of C can't be empty.");
	if (blue_color_length > 3)
		report_error(cub3d, "Blue color of C must be between \
		1-3 characters. (1 - 10 - 100).");
	c = 0;
	while (c < blue_color_length)
	{
		if (!ft_is_digit(blue_string[c]))
			report_error(cub3d, "Blue color of C must consist \
			of digit only. (0 - 9)");
		c++;
	}
	blue = ft_atoi(blue_string);
	if (blue < 0 || blue > 255)
		report_error(cub3d, "Blue color of C must be a value between 0-255.");
...
void	check_ceiling_color(t_c3d *cub3d, char *line)
{
	...
	if (line[0] == 'C')
	{
		...
		if (*cub3d->config->ceiling_color->string == '\0')
			report_error(cub3d, "C element cant' be empty.");
